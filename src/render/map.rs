use crate::render::{renderer::Renderer, textures::Textures};
use sdl2::{rect::Point, rect::Rect, render::Texture};
...
impl Renderer {
    ...
    pub fn create_map(&mut self, texture: &Textures) {
        // Drawing the 4 corners of the map
        {
            let _ = self.draw_rotated(&texture.road.get("Corner").unwrap(), 0, 0, 350, 350, 90.0);
            let _ = self.draw_rotated(
                &texture.road.get("Corner").unwrap(),
                650,
                0,
                350,
                350,
            ...
            let _ = self.draw_rotated(&texture.road.get("Corner").unwrap(), 0, 650, 350, 350, 0.0);
            let _ = self.draw_rotated(
                &texture.road.get("Corner").unwrap(),
                650,
                650,
...

        {
            ...
            let _ = self.draw_rotated(
                &texture.road.get("Center").unwrap(),
                350,
                350,
                300,
                300,
